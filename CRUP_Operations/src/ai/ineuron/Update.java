...
public class Update 
{
	public void update()
	{
		...
		try 
		{
			System.out.print("Enter eid number of the employee to update : ");
			...
			if(rs!=null)
			{
				...
				if(rs.getInt(1)==1)
				{
					System.out.println("1.name\t2.mobile\t3.company_name");
					...
					switch(option)
					{
						case 1: column_name="name";
						        break;
						case 2: column_name="mobile";
				        	    break;
						case 3: column_name="company_name";
		        	            break;
					}
					System.out.print("Enter the new data to be updated in the column "+column_name+" : ");
					String new_data=sc.nextLine();
					if(column_name.equals("mobile"))
					{
						p=null;
						if(p==null)
						{
							p=con.prepareStatement("update employee set "+column_name+" = ? where eid=?");
						}
						if(p!=null)
						{
				...
				{
					System.out.println("eid number not exist in the database");
				}
			}
			con.close();
		} 
